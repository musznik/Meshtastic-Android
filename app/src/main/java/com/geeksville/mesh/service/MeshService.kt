...
class MeshService : Service(), Logging {
    ...
    private fun rememberDataPacket(dataPacket: DataPacket, updateNotification: Boolean = true) {
        ...
        debug("processing rememberDataPacket")
        ...
        debug(String(dataPacket.bytes!!, Charsets.UTF_8))
        debug("processing data type")
        debug(dataPacket.dataType.toString())
            ...
            if (dataPacket != null) {
                ...
                when (data.portnumValue) {
                    ...
                    Portnums.PortNum.PRIVATE_APP_VALUE -> {
                        if (fromUs) return
                        EventBus.getDefault().post(DataPacketEvent(dataPacket))
                    }
